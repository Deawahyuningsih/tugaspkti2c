...
function renderCart() {
  ...
  updateStruk(); // Tambahan: update struk setelah renderCart
}
...
function checkout() {
  ...
  const confirmation = confirm(
    `Total belanja Anda: Rp ${totalPrice.toLocaleString("id-ID")}\n\nLanjutkan checkout?`
  );
...
// Tampilkan struk belanja otomatis
function updateStruk() {
  const output = document.getElementById("outputStruk");
  if (!output) return;
  ...
  cart.forEach((item, i) => {
    ...
    total += subtotal;
    struk += `${i + 1}. ${item.name} x ${item.qty} = Rp ${subtotal.toLocaleString("id-ID")}\n`;
  });
